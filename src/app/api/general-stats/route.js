...
async function computeGeneralStatsFromDatabase(season, raceSlug, sessionType) {
  ...
  try {
    ...
    // ... KEEP ALL your existing data processing logic EXACTLY the same ...
    ...
    const tempData = tempResults.rows.map(row => {
      // ... KEEP ALL your existing temperature processing logic ...
    ...
    // Format the data for the response - KEEP ALL your existing formatting logic
...
export async function GET(request) {
  try {
    ...
    // Generate cache key
    const cacheKey = getCacheKey.generalStats(season, raceSlug, sessionType);
    ...
    return cachedApiResponse(
      ...
      async () => {
        ...
        return await computeGeneralStatsFromDatabase(season, raceSlug, sessionType);
      },
      CACHE_DURATIONS.GENERAL_STATS // 6 hours cache
    );
