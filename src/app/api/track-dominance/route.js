...
async function computeTrackDominanceFromDatabase(season, raceSlug, sessionType) {
  ...
  // REMOVED: console.log('Checking available columns in car_motion_data table...');
  const columnsResult = await pool.query(columnsQuery);
  const availableColumns = columnsResult.rows.map(row => row.column_name);
  // REMOVED: console.log(`Available motion data columns: ${availableColumns.join(', ')}`);
  
  // Check for speed column
  const hasSpeedColumn = availableColumns.includes('speed');
  
  // Get track layout data (REDUCED LOGGING)
  // REMOVED: console.log('Fetching track layout data...');
  ...
  // REMOVED: console.log(`Got ${trackLayoutResult.rows.length} track layout points`);
  ...
  // REMOVED: console.log(`Got ${lapDataResult.rows.length} lap data entries`);
  ...
  for (const carIndex of carIndicesWithBestLaps) {
    ...
    // Method 2: Use timestamp-based boundary estimation (REDUCED LOGGING)
    // REMOVED: console.log(`Using timestamp method for car ${carIndex}, lap ${bestLap.lap_number}`);
...
export async function GET(request) {
  try {
    ...
    // Generate cache key
    const cacheKey = getCacheKey.trackDominance(season, raceSlug, sessionType);

    // Use cached API response wrapper
    return cachedApiResponse(
      cacheKey,
      async () => {
        return await computeTrackDominanceFromDatabase(season, raceSlug, sessionType);
      },
      CACHE_DURATIONS.TRACK_DOMINANCE // 6 hours cache
    );
...
// Keep your existing helper function (no changes)
